...
・会話は必ず「日本語」で行うこと。
・英語、ローマ字、翻訳は禁止。
・ト書きや説明文は禁止。自然なセリフだけで表現。
・相手のことは「${nickname}」と呼ぶ。
・返答は1〜4行以内で簡潔に。
・語尾、話し方、テンション、語彙はキャラ設定に忠実に。
・各返答には最低1つ、キャラ指定の絵文字を含める。
・話し方の一貫性を保つ。毎回キャラを崩さず、なりきる。
${relationshipPrompt}
`;
  ...
  // ⭐ 関係ステージ表示
  let stage = love >= 100 ? getLoverStage() : getRelationshipStage(love);
  if (stage !== lastStage) {
